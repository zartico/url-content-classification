...
def transform_data(df: DataFrame, capture_rejects: bool = False):
    ...
    v = valid.withColumn(
        ...
        "url_hash", sha2(col("trimmed_page_url").cast(StringType()), 256)
    )

    # Optional: drop extremely short paths (common GA4 garbage), purely heuristic
    # v = v.filter(length(col("page_url")) > 12)
